...
fn main() {
    ...
    match process_args(&args) {
        Ok(args) => {
            ...
            if args.targets_file.is_some() {
                println!("Targets file detected");
            } else {
                let sheet_id = args.sheet_id.unwrap();
                let sheet_range = args.sheet_range.unwrap();
                let google_cert = args.google_cert;
                tokio::runtime::Runtime::new().unwrap().block_on(async {
